...
ME включён по умолчанию. Telego подготавливает и проверяет соединения ME в фоне. До их готовности новые клиенты используют прямые соединения с DC.

Для работы ME разрешите HTTPS-доступ к `core.telegram.org` и TCP-доступ к адресам ME из артефактов Telegram. Приватным прямым сокетам также нужен UDP-доступ к STUN или правильный `nat-ip`. NAT для прямых соединений должен сохранять исходящие TCP-порты.

Для изменения параметров ME используйте необязательный раздел:

```toml
[middle-end]
# enabled = false # Раскомментируйте, чтобы отключить ME и фоновые попытки подключения.
...
После изменения этого раздела перезапустите Telego. Если раздел или параметр `enabled` отсутствует, ME включён. Явный `enabled = false` отключает ME.

После обновления удалите существующий `enabled = false`, чтобы использовать значение по умолчанию. Без этого переопределения ME запускается автоматически.

Логи уровня INFO сообщают, установлено ли соединение ME, недоступно ли оно или отключено явно. При неудачной подготовке новые клиенты используют прямой маршрут, а Telego повторяет попытку в фоне.

Telego заменяет отказавшее физическое соединение отдельно. Исправные привязки и пулы других DC продолжают работать.

Проверки и восстановление каждого физического слота работают независимо. Медленный слот не останавливает проверки остальных слотов.

Перед приёмом привязок замена должна завершить рукопожатие и вернуть RPC pong с соответствующим идентификатором. На подготовку замены отводится 10 секунд.
...
После ротации артефактов исправные старые привязки сохраняют свои соединения и проверки без срока принудительного закрытия. Старые поколения не восстанавливают и не обновляют соединения.

Одновременно Telego допускает не более двух живых менеджеров поколений, включая кандидата. Перед подготовкой следующего кандидата Telego при необходимости закрывает самое старое поколение.

Это закрытие ради ограничения ресурсов может прервать старые привязки. Неудачная подготовка кандидата сохраняет активное поколение, но не восстанавливает уже прерванные привязки.
...
Для максимальной производительности WEB используйте `websocket-lanes`. Установщик выбирает этот транспорт по умолчанию и настраивает передачу WebSocket Upgrade в Nginx.

Если параметр `carrier` отсутствует, Telego использует `https` для совместимости. Автоматического переключения транспорта нет.
...
enabled = false
# hostname = "proxy.example.com"
carrier = "websocket-lanes" # Рекомендуется для максимальной производительности WEB.
# bind-to = "127.0.0.1:8080"
...
| `telego_middleend_forced_retirement_total` | Counter | Принудительные закрытия поколений ради ограничения ресурсов, по `reason`: `artifact_capacity` или `recovery_capacity` |
| `telego_middleend_forced_retirement_affected_bindings_total` | Counter | Привязки, прерванные принудительным закрытием поколения, по `reason` |
| `telego_middleend_diagnostic_records_dropped_total` | Counter | Диагностические записи, которые переполненный журнал не принял. Метрика не имеет лейблов. |
