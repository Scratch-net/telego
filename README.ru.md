...
> **Telegram заблокирован?** TeleGO оформляет трафик прокси как TLS-записи и направляет нераспознанные подключения на маскирующий хост. [Быстрый старт](#быстрый-старт)
...
- **Владение буферами** — исходящий буфер остаётся в учёте до отправки или освобождения
- **Пул буферов** — повторное использование буферов там, где это допускает их время жизни
...
Публичный сервер и клиент ME работают на gnet. Telego поддерживает по четыре физических соединения для каждого идентификатора DC с учётом его знака.
...
Перед включением ME разрешите HTTPS-доступ к `core.telegram.org` и TCP-доступ к адресам ME из артефактов Telegram. Приватным прямым сокетам также нужен UDP-доступ к STUN или правильный `nat-ip`. NAT для прямых соединений должен сохранять исходящие TCP-порты.
...
Свободные соединения становятся кандидатами на обновление через 45–60 секунд. Telego завершает рукопожатие и RPC-проверку замены до закрытия текущего соединения.

Текущее соединение остаётся доступным во время подготовки. Новая клиентская привязка отменяет замену.
...
Диагностика отключена по умолчанию. Для её включения задайте `[metrics].diagnostics = true` и перезапустите Telego.

Диагностика требует буквальный loopback IP-адрес и ненулевой порт. Не публикуйте её маршруты через Nginx или другой публичный прокси.

Подробнее: [закрытая диагностика процесса](README.md#private-runtime-diagnostics), включая свежую проверку утечек горутин в Go 1.27.

Горячая перезагрузка меняет только `[general].log-level` и `[performance].idle-timeout`. Остальные изменения конфигурации требуют перезапуска.
...
Локальная замена gnet не позволяет установить Telego через `go install …@version`.
Используйте сборку из исходников или готовый бинарник.
...
| `telego_middleend_slot_refreshes_active` | Gauge | Кандидаты на обновление по роли поколения и DC, включая освобождение ресурсов |
| `telego_middleend_slot_refresh_total` | Counter | Результаты обновления по DC: `success`, `failure` или `canceled` |
| `telego_middleend_zero_ready_transitions_total` | Counter | Потери всех готовых соединений DC после начала приёма привязок |
