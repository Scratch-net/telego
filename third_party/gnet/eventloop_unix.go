...
// Telego local modification: explicit registration ownership and shutdown admission. See TELEGO.md.
package gnet
...
type eventloop struct {
	// Telego: pending enrollment descriptor ownership; see TELEGO.md.
...
func (el *eventloop) Register(ctx context.Context, addr net.Addr) (<-chan RegisteredResult, error) {
	...
	return el.enroll(ctx, nil, addr, FromContext(ctx))
}

// Enroll transfers ownership only when submission succeeds (the immediate
// error is nil). Its result then becomes terminal even if this loop exits.
func (el *eventloop) Enroll(ctx context.Context, c net.Conn) (<-chan RegisteredResult, error) {
	...
	return el.enroll(ctx, c, c.RemoteAddr(), FromContext(ctx))
}
...
func (el *eventloop) enroll(caller context.Context, c net.Conn, addr net.Addr, metadata any) (resCh chan RegisteredResult, err error) {
	if !el.engine.beginEnrollment() {
		return nil, errorx.ErrEngineInShutdown
	}
	resCh = make(chan RegisteredResult, 1)
	err = goroutine.DefaultWorkerPool.Submit(func() {
		defer el.engine.enrollments.Done()
		result := el.enrollConnection(caller, c, addr, metadata, queue.LowPriority, false)
		resCh <- result
		close(resCh)
	})
	if err != nil {
		el.engine.enrollments.Done()
		close(resCh)
	}
	return
}
...
func (el *eventloop) register(a any) error {
	if pending, ok := a.(*registration); ok {
		return pending.run()
	}
	return el.register0(a.(*conn))
}
