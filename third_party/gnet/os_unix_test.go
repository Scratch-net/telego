...
// Telego local modification: skip device-specific IPv6 tests only when the
// selected interface has no IPv6 address, before starting their server.
	...
	t.Run("IPv6", func(t *testing.T) {
		t.Run("TCP", func(t *testing.T) {
			...
			if errors.Is(err, errNoInterfaceIP) {
				t.Skipf("device-specific IPv6 test requires an IPv6 address on %s: %v", dev, err)
			}
			if err != nil {
				t.Fatal(err)
			}
