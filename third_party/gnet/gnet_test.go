...
// Telego local modification: UDP proxy tests use ephemeral backend ports and
// release partially prepared backends. The upstream traffic matrix is unchanged.
...
func testUDPProxyServer(t *testing.T, addr string, backendServers []string, multicore, et bool) {
	...
	srv := udpProxyServer{
		...
		ListenerNet:    network,
		ListenerAddr:   address,
		backendServers: append([]string(nil), backendServers...),
		packet:         []byte("andypan"),
		packetSize:     datagramLen,
	}
	...
	for i, backendServer := range backendServers {
		...
		if strings.HasPrefix(network, "udp") {
			...
			require.NoError(t, err, "ListenUDP error")
			netServers = append(netServers, c)
			srv.backendServers[i] = network + "://" + c.LocalAddr().String()
			backends.Go(func() error {
				startUDPEchoServer(t, c)
				return nil
			})
